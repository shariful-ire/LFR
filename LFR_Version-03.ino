...
void startFollow() {
  ...
  while (1) {
    ...
    // This is the core turn fix: at base_speed alone, the same gentle gains
    // used for tiny wobble corrections are too weak for a real turn, so the
    // robot runs straight through it. Instead of a hard on/off switch (which
    // caused a jerky snap the moment a threshold was crossed), turnFactor
    // scales continuously from 0 (dead straight) to 1 (sharpest possible
    // turn), so small curves get a small boost (stays smooth) and sharp
    // corners get the full boost + slowdown (so the turn isn't missed).
