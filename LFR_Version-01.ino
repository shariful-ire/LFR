...
void startFollow() {
  ...
  while (1) {
    ...
    if (sumOnSensor == 0) {
      ...
      // Turn toward the side the line was last seen on, not blindly right
      int8_t searchDir = (lastTurnDirection >= 0) ? 1 : -1;
    ...
    // last known error direction until sensors partially clear again.
    if (sumOnSensor == sensorNumber) {
      int8_t dir = (lastTurnDirection >= 0) ? 1 : -1;
    ...
    // FIX: use float division so line_position isn't stepped/quantized
    line_position = (float)sensorWight / (float)sumOnSensor;

    error = center_position - line_position;
    lastTurnDirection = (error >= 0) ? 1 : -1;
    ...
    float correctionF = (error * kp) + (derivative * kd * 0.001f);
    int32_t correction = (int32_t)correctionF;

    right_motor_correction = constrain((int32_t)base_speed + correction, -255, 255);
    left_motor_correction  = constrain((int32_t)base_speed - correction, -255, 255);
